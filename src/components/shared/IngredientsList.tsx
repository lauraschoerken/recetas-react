...
export function IngredientsList({ ingredients, onChange }: IngredientsListProps) {
  ...
  const searchIngredients = async (query: string, inputId: string) => {
    ...
    try {
      const results = await api.get<Suggestion[]>(`/ingredients/search?q=${encodeURIComponent(query)}`);
  ...
  const handleNameChange = (id: string, value: string) => {
    ...
    onChange(ingredients.map(ing => 
      ing.id === id 
        ? { ...ing, name: capitalizedValue, isFromDatabase: false, conversions: undefined, baseUnit: undefined, variants: undefined, variantId: undefined, variantName: undefined } 
        : ing
    ));
  ...
  const handleVariantsChange = (ingredientId: string, variants: IngredientVariant[], selectedVariantId?: number) => {
                ...
                <div className="ingredients-table-row">
                  <div className="ingredient-input-wrapper">
                    {ing.name && !ing.isFromDatabase && (
                      <span className="new-ingredient-badge" title="Ingrediente nuevo - se creará al guardar">✨</span>
                    )}
                    <input
                      type="text"
                    ...
                    {ing.name && (
                      <>
                        <button
                          ...
                          onClick={() => setShowConversionsId(showConversionsId === ing.id ? null : ing.id)}
                        ...
                        <button
                          ...
                          title={ing.isFromDatabase ? "Ver/editar estados y macros" : "Crear ingrediente y añadir macros"}
                        >
                          📊
                        </button>
                      </>
                    ...
                    {ing.conversions.length > 0 ? (
                      <ul className="conversions-list">
                        {ing.conversions.map(c => (
                          <li key={c.id}>1 {c.unitName} = {c.gramsPerUnit} {ing.baseUnit || 'g'}</li>
                        ))}
                      </ul>
                    ) : (
                ...
                {showMacrosId === ing.id && (
                  ing.databaseId && ing.variants ? (
                    <IngredientStatesPanel
                      ...
                      onVariantsChange={(variants, selectedVariantId) => handleVariantsChange(ing.id, variants, selectedVariantId)}
                    />
                  ) : (
                    <div className="ingredient-info-panel new-ingredient-panel">
                      <div className="info-panel-header">Ingrediente nuevo: {capitalizeFirst(ing.name)}</div>
                      <p className="new-ingredient-hint">Este ingrediente no existe en la base de datos. Se creará automáticamente al añadir estados/macros.</p>
                      <button
                        type="button"
