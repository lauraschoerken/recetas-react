...
export function HomeItemCard({ item, onUpdate, onDelete, onCook }: HomeItemCardProps) {
  ...
  useEffect(() => {
    if (showCookMenu && item.ingredientId && variants.length === 0) {
      ...
      ingredientService.getVariants(item.ingredientId)
        ...
        {isEditing ? (
          ...
          <div className="home-item-quantity">
            ...
            {((item.pendingPrepServings || 0) > 0 || (item.plannedMealServings || 0) > 0) && (
              <span className="home-item-projected" title="Te quedarán">({item.projectedTotal ?? item.quantity})</span>
            )}
            <span className="home-item-unit">{item.unit}</span>
            {((item.pendingPrepServings || 0) > 0 || (item.plannedMealServings || 0) > 0) && (
              <span className="home-item-changes">
                {(item.pendingPrepServings || 0) > 0 && (
                  <span className="change-add" title="A preparar">+{item.pendingPrepServings}</span>
                )}
                {(item.plannedMealServings || 0) > 0 && (
                  <span className="change-sub" title="Vas a consumir">−{item.plannedMealServings}</span>
                )}
              </span>
            )}
          </div>
      ...
      <div className="home-item-actions">
        {isEditing ? (
          ...
          <>
            ...
            <button className="btn-icon btn-icon-danger" onClick={() => onDelete(item.id)} title="Eliminar">
              <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
