...
import { Ingredient, UpdateIngredientData, UnitConversion, IngredientVariant, ingredientService } from '@/services/ingredient';
...
export function IngredientCard({ ingredient, onUpdate, onDelete, onConversionChange }: IngredientCardProps) {
      ...
      <div className="ingredient-card editing">
        <div className="ingredient-edit-form">
          ...
          <div className="image-url-row">
            ...
            />
            {imageUrl && (
              <img src={imageUrl} alt="Preview" className="image-preview-small" />
            )}
          </div>
  ...
  return (
    <div className="ingredient-card">
        ...
        <div className="variants-panel">
          <div className="variants-list">
            {variants.map((variant) => (
              <div key={variant.id} className={`variant-item ${variant.isDefault ? 'is-default' : ''}`}>
                {editingVariantId === variant.id ? (
                  <div className="variant-edit-form">
                    ...
                    <div className="variant-edit-actions">
                      <button className="btn btn-sm btn-outline" onClick={cancelEditVariant}>Cancelar</button>
                      <button className="btn btn-sm btn-primary" onClick={saveVariant}>Guardar</button>
                    </div>
                  </div>
                ) : (
                  <>
          ...
          {editingVariantId === -1 ? (
            <div className="variant-edit-form new-variant-form">
              ...
              <div className="variant-edit-actions">
                <button className="btn btn-sm btn-outline" onClick={cancelEditVariant}>Cancelar</button>
                <button className="btn btn-sm btn-primary" onClick={addNewVariant} disabled={!variantName.trim()}>Añadir</button>
              </div>
            </div>
          ) : (
            <button 
              ...
              <div className="conversions-list">
                {conversions.map((conv: UnitConversion) => (
                  <div key={conv.id} className="conversion-item">
                    <span>1 {conv.unitName} = {conv.gramsPerUnit}{ingredient.unit}</span>
                    <button 
                      className="btn-icon-small"
                  ...
                  >
                    ...
                    {conversions.map((conv: UnitConversion) => (
                      <option key={conv.id} value={conv.unitName}>
                        {conv.unitName} ({conv.gramsPerUnit}{ingredient.unit})
                      </option>
                    ))}
                  </select>
                </label>
      ...
      <div className="ingredient-actions">
        ...
        <button className="btn-icon btn-icon-danger" onClick={() => onDelete(ingredient.id)} title="Eliminar">
          <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
