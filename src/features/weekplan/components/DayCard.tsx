...
export function DayCard({ date, dateStr, dayName, plans, onRemove, onMovePlan, onCook, onConsume }: DayCardProps) {
    ...
    return (
      ...
      >
        <div className="day-card-item-header">
          {!isCompleted && <span className="drag-handle" title="Arrastrar para mover">⋮⋮</span>}
          {isCompleted && <span className="done-icon" title={isPrep ? 'Cocinado' : 'Consumido'}>✓</span>}
          ...
          <>
            ...
            {preps.length > 0 && (
              <div className="day-card-section">
                ...
                <ul className="day-card-list">
                  {preps.map((plan) => renderPlanItem(plan, true))}
                </ul>
              </div>
            )}
          </>
